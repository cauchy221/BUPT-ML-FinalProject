...
def load_data(data_path):
    if 'test' in data_path:
        ...
        with open(data_path, encoding='utf-8') as f:
            for line in f.readlines()[1:]:
                line_lst = line[:-1].split(',')
                results["sentiment"].append(int(line_lst[0]))
                results["text"].append(line_lst[1])
            results = Dataset.from_dict(results)
    return results
...
def Compute_Accuracy(preds_lst, refs_lst):
    total = len(preds_lst)
    acc = 0
    for i in range(total):
        if int(preds_lst[i]) == int(refs_lst[i]):
            acc += 1
    return acc / total
...
def main():
    ...
    def preprocess_function(examples):
        texts = ((examples['text'],))
        result = tokenizer(*texts, max_length=args.max_source_length, truncation=True, padding=True)
        result['labels'] = [label_to_id[l] for l in examples['sentiment']]
        return result
    ...
    for epoch in range(args.num_train_epochs):
        ...
        acc = Compute_Accuracy(eval_preds, eval_labels)
        result = {'epoch': epoch, 'acc': acc}
        logger.info(result)
        wandb.log(result)

        eval_output = []
        for i in range(0, len(eval_preds)):
            # print(eval_preds[i], type(eval_preds[i]))
            # print(eval_labels[i], type(eval_labels[i]))
            eval_output.append(str(eval_preds[i].int()) + '\t' + str(eval_labels[i].int()))
        ...
        for src in raw_datasets['test']['text']:
            with torch.no_grad():
                input_ids = tokenizer.encode(
                    src, max_length=args.max_source_length, truncation=True, padding=True, return_tensors='pt')
                preds = model(
                    input_ids.cuda()).logits.argmax(dim=-1)
            test_output.append(str(preds.int()))
